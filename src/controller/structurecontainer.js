...
class ControllerStructureContainer {

  static run(structure) {
    if(structure.memory.taskId)
      return this.updateTask(structure)

    if(structure.store.getFreeCapacity(RESOURCE_ENERGY) > structure.store.getCapacity(RESOURCE_ENERGY) * 0.25)
      this.createTask(structure)
  }

  static createTask(structure) {
    const taskId = ControllerRoom.createTask(structure.room, "harvestRequired", {
      priority: 9,
      target: structure.id,
...
export default Export(ControllerStructureContainer)
